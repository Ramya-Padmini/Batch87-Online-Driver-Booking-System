...
int delete_user_data()
{
	...
	else
	{
		...
		if(!ptr)
		{
			printf("\nUser's aadhar number %d not found for delection\n",uad1);
			return EXIT_FAILURE;
		}
		prev->next=ptr->next;
		free(ptr);
...
int edit_driver_data()  
{
	...
	if(!ptr1)
	{
		...
		printf("\nUser's aadhara %ld is not found\n",dlno1);
		return EXIT_FAILURE;
	}
	printf("Old name %s address %s age %d and phone number %s",ptr1->nam,ptr1->adrs,ptr1->da,ptr1->dphno);
...
int view_list_data() 
{
	...
	printf("\n Route no        Starting point       Ending point      Distace      Status of driver");
